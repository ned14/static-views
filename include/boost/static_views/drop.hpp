...
namespace detail
{
  template <class View> struct drop_impl : view_adaptor_base<drop_impl<View>, View>
  {
    ...
    ///           contain \f$ max(\text{xs.get().size()} - \text{b}, 0)\f$
    ///           elements.
    ///
    /// \verbatim embed:rst:leading-slashes
    /// .. note::
    ...
    /// \endverbatim
    explicit BOOST_STATIC_VIEWS_CONSTEXPR drop_impl(View &&xs, std::size_t const b) BOOST_STATIC_VIEWS_NOEXCEPT_IF(utils::all(std::is_nothrow_constructible<typename drop_impl::view_adaptor_base_type, View &&>::value,
    ...
    {
      static_assert(std::is_nothrow_constructible<typename drop_impl::view_adaptor_base_type, View &&>::value, "");
    }
    ...
    /// fails.
    /// \endverbatim
    BOOST_STATIC_VIEWS_CONSTEXPR auto size() const BOOST_STATIC_VIEWS_NOEXCEPT_IF(drop_impl::is_noexcept_parent_size()) { return this->parent().size() - _b; }

    /// \brief "Maps" index \p i to the corresponding index in the parent
    /// view.

    /// Let `xs` be of type #drop_impl. The following relation then holds
    /// \f[
    ...
    BOOST_STATIC_VIEWS_CONSTEXPR auto map(std::size_t const i) const { return i < size() ? (_b + i) : ((void) make_out_of_bound_error("`i < size()` not satisfied."), _b + i); }

  private:
    // friend struct BOOST_STATIC_VIEWS_NAMESPACE::view_adaptor_core_access;
    std::size_t _b;

    static constexpr auto is_noexcept_parent_size() noexcept { return noexcept(std::declval<drop_impl const &>().parent().size()); }
  };

  struct make_drop_impl
  {
    template <class View> BOOST_STATIC_VIEWS_CONSTEXPR auto operator()(View &&xs, std::size_t const b) const BOOST_STATIC_VIEWS_AUTO_RETURN_NOEXCEPT(drop_impl<std::decay_t<View>>{std::forward<View>(xs), b})
  };
...
/// first ``n`` elements dropped. Type of ``xs`` may be anything as long as it
