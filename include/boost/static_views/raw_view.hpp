...
namespace detail
{
  template <class Sequence> struct raw_view_impl : view_base
  {
    ...
    ///   of data.
    /// \endverbatim
    explicit BOOST_STATIC_VIEWS_CONSTEXPR raw_view_impl(Sequence &xs) noexcept : _xs{&xs} {}
    ...
    BOOST_STATIC_VIEWS_CONSTEXPR
    BOOST_STATIC_VIEWS_DECLTYPE_AUTO operator[](std::size_t const i) const { return BOOST_STATIC_VIEWS_LIKELY(i < size()) ? sequence_traits<sequence_type>::at(*_xs, i) : (make_out_of_bound_error("Index `i` exceeds size of sequence."), sequence_traits<sequence_type>::at(*_xs, i)); }
    ...
    #endif
            {
                return BOOST_STATIC_VIEWS_LIKELY(i < size())
  ...
  private:
    using sequence_type = std::remove_const_t<std::remove_reference_t<Sequence>>;
    Sequence *_xs;
  };
  ...
  {
    template <class Sequence> BOOST_STATIC_VIEWS_CONSTEXPR auto operator()(Sequence &sequence) const BOOST_STATIC_VIEWS_AUTO_RETURN_NOEXCEPT(raw_view_impl<Sequence>(sequence))
  };
  /// \endcond
