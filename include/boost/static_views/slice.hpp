...
namespace detail
{
  ...
  template <class DropTake> struct slice_impl : DropTake
  {
    ...
    /// \endverbatim
    template <class View>
    BOOST_STATIC_VIEWS_CONSTEXPR slice_impl(View &&xs) BOOST_STATIC_VIEWS_NOEXCEPT_IF(std::is_nothrow_constructible<DropTake, View &&>::value)
    ...
    BOOST_STATIC_VIEWS_DECLTYPE_AUTO parent() const { return static_cast<DropTake const *>(this)->parent().parent(); }
  };

  struct make_slice_impl
  {
    template <class View>
    BOOST_STATIC_VIEWS_FORCEINLINE BOOST_STATIC_VIEWS_CONSTEXPR auto operator()(View &&xs, std::size_t const b, std::size_t const e) const BOOST_STATIC_VIEWS_AUTO_RETURN_NOEXCEPT(slice_impl<std::decay_t<decltype(drop(b)(take(e)(std::forward<View>(xs).get())))>>{drop(b)(take(e)(std::forward<View>(xs).get()))})
  };
