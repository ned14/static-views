...
namespace detail
{
  template <class View> struct take_impl : view_adaptor_base<take_impl<View>, View>
  {
    ...
    /// \endverbatim
    explicit BOOST_STATIC_VIEWS_CONSTEXPR take_impl(View &&xs, std::size_t const n) BOOST_STATIC_VIEWS_NOEXCEPT_IF(utils::all(std::is_nothrow_constructible<typename take_impl::view_adaptor_base_type, View &&>::value,
    ...
    BOOST_STATIC_VIEWS_CONSTEXPR auto map(std::size_t const i) const { return i < size() ? i : ((void) make_out_of_bound_error("`i < size()` not satisfied."), i); }

  private:
    // friend struct BOOST_STATIC_VIEWS_NAMESPACE::view_adaptor_core_access;
    std::size_t _n;

    static constexpr auto is_noexcept_parent_size() noexcept { return noexcept(std::declval<take_impl const &>().parent().size()); }
  };

  struct make_take_impl
  {
    template <class View> constexpr auto operator()(View &&xs, std::size_t const n) const BOOST_STATIC_VIEWS_AUTO_RETURN_NOEXCEPT(take_impl<std::decay_t<View>>{std::forward<View>(xs), n})
  };
